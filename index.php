...
<body>
  <header>
    <h1><i class="fas fa-seedling"></i> Crop-Based Pesticide Store Locator</h1>
    <p>Find the right pesticides for your crop and nearby stores</p>
  </header>
  ...
  <div class="container">
    <section class="search-section">
      <div class="search-container">
        <div class="search-box">
          ...
          <button onclick="searchPesticide()"><i class="fas fa-search"></i> Search</button>
        </div>
    ...
    <div class="content">
      <section class="results-section">
        <div id="resultsCount">Search for a crop to find stores</div>
  ...
  <footer>
    <p>&copy; 2025 Crop-Based Pesticide Store Locator. All rights reserved.</p>
  </footer>
  ...
  <script>
    ...
    async function searchPesticide() {
      ...
      try {
        const res = await fetch(`get_pesticides.php?crop=${encodeURIComponent(crop)}`);
    ...
    function displayStores(stores) {
      ...
      stores.forEach((store, i) => {
        const { name, address, latitude, longitude, pesticides } = store;
        let distanceText = "";
        if (userLocation && latitude && longitude) {
          const dist = calculateDistance(userLocation.lat, userLocation.lng, latitude, longitude);
          distanceText = `<div><i class="fas fa-location-arrow"></i> ${dist.toFixed(1)} km away</div>`;
        }
        const storeCard = document.createElement("div");
        storeCard.className = "store-card";
        storeCard.innerHTML = `
          <h3><i class="fas fa-store"></i> ${name}</h3>
    ...
    function addMarkersToMap(stores) {
      stores.forEach((store, i) => {
        ...
        const marker = L.marker([latitude, longitude])
          .bindPopup(`<b>${name}</b><br>${address}<br><button onclick="window.focusStore(${i})">View</button>`)
          .addTo(map);
        markers.push(marker);
      });
    }
